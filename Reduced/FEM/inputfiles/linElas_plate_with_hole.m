function [model,coord,inz,bcond,load,lfun] = linElas_plate_with_hole(matpar)
% DESCRIPTION
%  input file for linear elastic plate with a hole containing required 
%  informations of the spatial discretization and applied loads
%
% INPUT
...
npe    = 4; % nodes per element: bilinear Q4-elements
ndf    = 2; % number of degrees of freedom per node
ptype  = 'plane_stress'; % problemtype 2D
elform = 'quad'; % element type: quadrilateral

% path to spatial discretization data
path = '../FEM/inputfiles/modelData_linElas_plate-with-hole/';

% node coordinates
coord = readmatrix(strcat(path,'linElas_plate_nodeCoordinates.csv'));
nnode = size(coord,1);
...
% incidence matrix
inz = readmatrix(strcat(path,'linElas_plate_elementIncidences.csv'));
nelem = size(inz,1);
...
% symmetry boundary condition at right edge (x_1 == 10) 
...
% initialize
load = zeros(nnode,2*ndf+1);

% force boundary condition at left edge (x_1 == 0)
global_nodeID     = idx(abs(coord(:,1))<=10*eps);
idx1              = 1;
idx2              = idx1-1+length(global_nodeID);
load(idx1:idx2,1) = global_nodeID;
load(idx1:idx2,2) = 2; % applied load function in x_1-direction
load(idx1:idx2,4) = -1; % load factor for nodal force in x_1-direction
...
lfun{2} = @(t)(1500/56); % prescribed nodal force [N]
...
% linear elasticity with material parameters
%  matpar(1) - Young modulus [N/mm^2]
%  matpar(2) - Poisson ratio [-]
model.matpar = matpar;
model.matfun = @stct_linElasticity_2D;

% linear elasticity has no internal variables
model.nq = 0;
