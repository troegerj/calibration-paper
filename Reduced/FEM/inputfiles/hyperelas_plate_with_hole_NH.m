function [model,coord,inz,bcond,load,lfun] = hyperelas_plate_with_hole_NH(matpar)
% DESCRIPTION
%  input file for hyperelastic plate with a hole using Neo-Hookean model
...
lfun{2} = @(t)(0.2*t); % prescribed displacement in x_1-direction [mm]
lfun{3} = @(t)(0.4*t); % prescribed displacement in x_2-direction [mm]
...
% Neo-Hookean hyperelasticity with material parameters
%  matpar(1) - bulk modulus [N/mm^2]
%  matpar(2) - c_10 [N/mm^2]
model.matpar = matpar;
model.matfun = @stct_NeoHooke;
...
% output time
out.time = linspace(0.25,tend,tend/0.25)'; % every 0.25 s 
