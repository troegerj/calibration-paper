...
class BFGSOptimizerState(NamedTuple):
    ...
    is_converged: Union[bool, JNPBool]
    params_k: JNPPyTree
    func_k: JNPFloat
    grad_k: JNPArray
...
def BFGS(
    c1: float = 1e-4, c2: float = 0.9, line_search_maxiter: int = 20, gtol: float = 1e-5
    ...
    def init_func(
        ...
        dim_params = params_0.shape[0]
        dtype_params = params_0.dtype
        inv_hessian_0 = jnp.eye(dim_params, dtype=dtype_params)
    ...
    def update_func(
        ...
        line_search_results_k = line_search(
            ...
            c1=c1,
            c2=c2,
            max_bracketing_iters=line_search_maxiter,
            max_zoom_iters=30,
        )
        ...
        helper_a = jnp.expand_dims(param_update_k, 1) * jnp.expand_dims(
            grad_update_k, 0
        )
        helper_b = jnp.eye(dim_params, dtype=params_k.dtype) - rho_k * helper_a
        # Subscripts are "ij,jk,kl" and not "ij,jk,kl" as the third operand is transposed.
        inv_hessian_kp1 = _einsum(
            "ij,jk,lk", helper_b, inv_hessian_k, helper_b
        ...
        is_terminated = _check_if_terminated(status)
        num_func_evals = (
    ...
    def _check_if_terminated(status: int) -> JNPBool:
        return status != 1
