...
from calibrationpinn.training.pinns.momentumEquation2D_E_nu import (
    ...
    MomentumEquation2DEstimates_E_nu,
)
...
class TestMomentumEquation2D_E_nu_planeStrain(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._shear_modulus = self._youngs_modulus / (2 * (1 + self._poissons_ratio))
        ...
        self._poissons_ratio_correction = 0.0
        youngs_modulus_correction_func = set_up_fake_youngs_modulus_correction(self)
        poissons_ratio_correction_func = set_up_fake_poissons_ratio_correction(self)
        ...
        params_poissons_ratio_correction = poissons_ratio_correction_func.init(
            PRNG_key, input=jnp.zeros(input_size)
        )
        self._parameters = MomentumEquation2DParameters_E_nu(
            net_ux=params_net_ux,
            net_uy=params_net_uy,
            youngs_modulus_correction=params_youngs_modulus_correction,
            poissons_ratio_correction=params_poissons_ratio_correction,
        )
        self._models = MomentumEquation2DModels_E_nu(
            net_ux=net_ux,
            net_uy=net_uy,
            youngs_modulus_correction=youngs_modulus_correction_func,
            poissons_ratio_correction=poissons_ratio_correction_func,
        )
        self._estimates = MomentumEquation2DEstimates_E_nu(
...
class TestMomentumEquation2D_E_nu_planeStress(unittest.TestCase):
    def __init__(self, methodName: str) -> None:
        super().__init__(methodName=methodName)
...
class TestCaseProtocol(Protocol):
    ...
    _poissons_ratio_correction: float
    _parameters: MomentumEquation2DParameters_E_nu
    _models: MomentumEquation2DModels_E_nu
    _estimates: MomentumEquation2DEstimates_E_nu
...
def set_up_fake_poissons_ratio_correction(
    test_case: TestCaseProtocol,
) -> HKTransformed:
    def model_func(input: JNPArray) -> JNPArray:
        model = ConstantFunction(
            output_size=1,
            func_value_init=hk.initializers.Constant(
                test_case._poissons_ratio_correction
            ),
            name="fake_poissons_ratio_correction",
        )
        return model(input)
